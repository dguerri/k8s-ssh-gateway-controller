...
func (r *TCPRouteReconciler) handleAddOrUpdate(ctx context.Context, req ctrl.Request, k8sRoute *gatewayv1alpha2.TCPRoute, routeDetails *routeDetails) (ctrl.Result, error) {
	// Add or Update.
	slog.With("function", "Reconcile", "tcpRoute", req.NamespacedName).Debug("adding or updating TCPRoute")
	// Add a finalizer so we can correctly clean up the route when it's deleted
	if !containsString(k8sRoute.Finalizers, getTCPRouteFinalizer()) {
		k8sRoute.Finalizers = append(k8sRoute.Finalizers, getTCPRouteFinalizer())
	...
	if err != nil {
		...
		if errors.As(err, &notReadyErr) || errors.As(err, &notFoundErr) {
			...
			writeRouteParentStatus(ctx, r.Client, k8sRoute, &k8sRoute.Status.RouteStatus,
				...
				pendingRouteConditions("Parent Gateway is not ready"), "tcpRoute")
	...
	writeRouteParentStatus(ctx, r.Client, k8sRoute, &k8sRoute.Status.RouteStatus,
		k8sRoute.Spec.ParentRefs[0], k8sRoute.Generation,
		acceptedRouteConditions(), "tcpRoute")
...
func (r *TCPRouteReconciler) handleDeletion(ctx context.Context, req ctrl.Request, k8sRoute *gatewayv1alpha2.TCPRoute, routeDetails *routeDetails) (ctrl.Result, error) {
	// Handle deletion
	...
	if containsString(k8sRoute.Finalizers, getTCPRouteFinalizer()) {
		...
		k8sRoute.Finalizers = removeString(k8sRoute.Finalizers, getTCPRouteFinalizer())
		if err := r.Update(ctx, k8sRoute); err != nil {
